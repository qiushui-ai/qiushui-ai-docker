...
class LangGraphService:
    ...
    def __init__(self, config_path: str = "langgraph.json"):
        # Default path can be overridden via QIUSHUIAI_AGENTS_CONFIG or by placing langgraph.json
    ...
    async def initialize(self):
        ...
        Resolution order:
        1) QIUSHUIAI_AGENTS_CONFIG env var (absolute or relative path)
        2) Explicit self.config_path if it exists
        3) langgraph.json in CWD
        4) langgraph.json in CWD (fallback)
        """
        # 1) Env var override
        env_path = os.getenv("QIUSHUIAI_AGENTS_CONFIG")
        resolved_path: Path
        if env_path:
        ...
        if not resolved_path.exists():
            raise ValueError(
                ...
                "QIUSHUIAI_AGENTS_CONFIG path, ./langgraph.json, or ./langgraph.json"
            )
