#!/usr/bin/env python3
"""Database migration management script for Qiushuiai Agents."""
import os
import sys
import subprocess
...
# Since this file is now in alembic/, parent.parent gets us to project root
...
def main():
    ...
    if len(sys.argv) < 2:
        print("""
🔧 qiushuiai-agents Database Migration Manager

Usage:
  python alembic/migrate.py <command> [options]
...
Examples:
  python alembic/migrate.py upgrade
  python alembic/migrate.py revision --autogenerate -m "Add user preferences"
  python alembic/migrate.py reset
        """)
        return
    ...
    if command == "init":
        ...
        if len(sys.argv) < 3:
            ...
            print("Usage: python alembic/migrate.py revision -m \"Your message\"")
            return
        ...
        print("Run 'python alembic/migrate.py' for help")
